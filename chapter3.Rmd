...
Unfortunately, there is no easy way to identify which rows of the source dataset *student_mat* correspond to these goofy values, to use them as replacements. This is because the rows in the two source datasets lack a unique identifier field (which IMHO is a methodological error). Even the combination of 13 columns that we used as student identifiers when merging the columns is not reliable -- there are rows with identical sequences of values in these columns. Therefore, to replace the goofy _studytime_ values with the original integer ones from *student_mat*, I made a temporary adjustment to the code I had used to combine the two datasets in the previous section, simply adding one condition to the *for*-loop to give _studytime_ the same treatment as the non-numeric variables:  
...
Much better. Now there are no ridiculously underrepresented groups. Having gotten started mutating variables, however, we should now make another little adjustment in order to render this data more informative -- this time without changing any of the actual values. Since _studytime_ and _studytime.adjusted_ are ordinal non-interval variables, it is not intuitively clear what their levels represent. In fact, the 1-4 scale is a bit misleading as it might lead us to assume that the levels are evenly spaced. This is easily fixed by converting our new _studytime.adjusted_ variable into a factor. A factor is a vector whose values are treated as categorical and can be named and ordered however we choose. In our case, we want to preserve the original ordering and simply name the levels informatively. This is easy in R:  
...
This data supports my fairly self-evident hypothesis that diligent students booze less than lazy ones.

###Predictor: romantic 
  
Alcohol lowers inhibitions and boosts libido. This is why it has been a powerful aid in human pair-bonding for aeons. In fact, this might just be the most important reason it is used in the first place, whether people admit it or not. Therefore, the prediction that people who are in a relationship, i.e. have already pair-bonded, have less reason to use alcohol was a fairly obvious one to make. As a binary variable, this one should be the easiest of the four to analyze.  
...
```
  ...
  It _is_ interesting, however, that it's not the most studious group that drinks the least. We might hypothesize that those who study 5-10h a week live happy, balanced lives and do not need to drink themselves silly to relax and have fun, whereas those who study massive amounts need more extreme measures, such as intoxicants, to unwind.
  
  Lastly, having a romantic partner proves statistically non-significant in predicting high alcohol use. The risk does decrease in a relationship, but only by 23%. P-value is .31; sample size is in the triple digits, i.e. large enough. I guess I was simply mistaken about this one.
...
```  
  ...
  Here's a how it looks:
```{r}
head(alcohol.practice)
```  
  
The probability of the target outcome for a given data point is calculated from the coefficients that the model calculated for each explanatory variable, including the constant (aka Intercept) We'll illustrate this by arbitrarily selecting a student from our dataset. For example, let's pick Student #100 in the dataset for analysis:

```{r}
alcohol.practice[100,]
```  
  
  In order to use our model to estimate the probability of this student being a heavy drinker, we must sum up all the logodds coefficients corresponding to his/her predictor variables (including Intercept), convert the sum into an odds ratio, and then convert that odds ratio into a probability. This is how: 
...
```  
  ...
  That's quite a bit of manual calculus to do, so fortunately there's a  __predict()__ function does all of it for us. We need only specify the model, the data, and the type of our outcome variable:
...
So it's slightly below 30%. An educated guesser would take this empirical probability and toss a correspondingly biased coin for each student in the dataset, resulting in a sequence of predictions like this:
...
Therefore, aside from prediction power, model economy is another key goal of regression analysis. We aim to explain as much of the variation of Y as possible with as few X-variables as possible, because every new X-variable we add to the model increases the amount of explaining we have to do -- we want our explanation of a given phenomenon to be as succinct as possible.  
  
  The AIC score provides a measure of this. It is a function of residual deviance and model complexity, i.e., inversely correlated with prediction power and model simplicity. The lower the AIC score, the better the model.
...
Say what? AIC and Residual Deviance deteriorated further. Even Training Error went up, all of which was to be expected from removing a clearly significant predictor. Yet Testing Error improved again???? I am stumped, I can't even. Basic common sense would suggest, however, that the better a model predicts outcomes, the better it is, no matter what any other fancy mathematical calculations might say. Therefore, we will keep playing this game for as long as Testing Error keeps improving. The next variable to go is _address_.
```{r}
Round8 <- logregr(alcohol, high_use ~ sex+goout+studytime)
```
WHOA stop right there! Enough! Now EVERYTHING got worse. That was clearly one step too far. We are finished. Let's draw a chart of how Testing Error and Training Error progressed throughout our elimination process:
```{r}
